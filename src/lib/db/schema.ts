...
// Donation configuration (budget, the one-time share answer, settlements) is service
// data, not reading data: plaintext jsonb, keyed by the same pseudonymous sync id and
// guarded by the same write-token hash, registered on first write. The doc's shape is
// donationDocSchema in @swdi/shared, parsed on every read since jsonb bytes are
// whatever was stored.
